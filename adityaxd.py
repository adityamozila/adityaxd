...
try:
	...
	color_rich = "[#afafff]"
	color_table = "#9F9F9F"
sys.stdout.write('\x1b]2; AdityaXD \x07')
try:
	os.mkdir('result')
except:
	pass
...
def waktu():
	now = datetime.now()
	hours = now.hour
	if 4 <= hours < 12:timenow = "ADITYA-XD"
	elif 12 <= hours < 15:timenow = "ADITYA-XD"
	elif 15 <= hours < 18:timenow = "ADITYA-XD"
	else:timenow = "ADITYA-XD"
	return timenow
def jalan(keliling):
	for mau in keliling + '\n':
...
{H2} Folow Instagram @aditya_impostor
    _    ____ ___ _______   __ _   __  ______  
   / \  |  _ |_ _|_   _\ \ / // \  \ \/ |  _ \ 
  / _ \ | | | | |  | |  \ V // _ \  \  /| | | |
 / ___ \| |_| | |  | |   | |/ ___ \ /  \| |_| |
/_/   \_|____|___| |_|   |_/_/   \_/_/\_|____/  """
...
def loadinglogin():
    ...
    for i in range(50):
        time.sleep(0.1)
        sys.stdout.write(f"\r{N}[{H}•{N}] {H}Jangan lupa follow ig aditya_impostor....{N} " + animation[i % len(animation)] +"\x1b[0m ")
        sys.stdout.flush()
    print("")
try:
...
class instagram:
	...
	def logo(self):
		...
		#	urut.append(Panel(f"{P2}Nama      : {H2}{nama}\n{P2}Username  : {H2}{self.username}\n",title=f"{M2}• {K2}• {H2}•{P2} {P2}Data {H2}• {K2}• {M2}•",width=80,padding=(0,3),style=f"{color_table}"))
			prints(Panel(f"{P2}\t      {H2}{nama}{P2} semangat sayang",width=80,style=f"{color_table}"))
...
[white][[blue]03[white]]. Crack Dari Mengikuti           [white][[red]E[white]]. [red]LogOut """,title=f"[{P2} Pilihan Menu {hapus}]",width=80,padding=(0,4),style=f"{color_table}"))
	def BUG(self):
		jalan(f"㋛ Tunggu Sedang Proses Menuju WhatsApp Admin");time.sleep(5);os.system("xdg-open https://wa.me/+6283843312512?");time.sleep(5);exit()
